    ...
    while(found != num_filters) {
      ...
      for(i = 0; i < count; i++) {
        if(! (pollfd[i].revents & POLLIN)) {
          ...
          if (pollretries[i] >= 20) {
            dprintf3("start: giving up wait for pid on filter %d, no data found...\n",i);
            found++;
          }
          continue;
        }
  ...
  while(1) {
    ...
    if((sid_ll = find_pid(&sid_data->sidlist, dmxcmd->pid))) {
      ...
      if (sid_data->sendmsg) {
        ...
        dprintf3("Didn't find sid for pid: %d\n", dmxcmd->pid);
      }
    } else {
        dprintf3("Didn't find sid for pid: %d\n", dmxcmd->pid);
    }
...
static struct option *parseopt_sid(arg_enum_t cmd)
{
  ...
  if(cmd == ARG_HELP) {
    printf("   --sid-filt <num>  : Maximum number of open filters (default 50)\n");
